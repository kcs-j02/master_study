...
BenchResult run_taskflow_cuda(
  ...
  /*
   * 3. 最大レベル幅からstream数を決める（最大5本）。
   * この段階ではSMの分割や資源割り当てを行わない。
   */
  const int stream_count =
      decide_stream_count(levels, max_stream_count);

  /*
   * 4. 既存手法の規則でタスクをstreamへ割り当てる。
   * 通常CUDA stream間に専有SM数の違いはないため、
   * 全streamを同じ性能としてスケジューリングする。
   */
  ...
  std::cout << "===== Existing method configuration =====\n";
  ...
  for (int stream_id = 0;
    ...
    std::cout
        ...
        << " SM scheduling reference (shared)\n";
  }
  ...
  /*
   * 6. Green Contextを作らず、通常CUDA streamを生成する。
   */
  RuntimeResources rr;
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    std::cout << "sequential gpu_kernel_ms: "
              << sequential_gpu_kernel_ms
              << " ms\n";
    std::cout << "existing   gpu_kernel_ms: "
              << result.gpu_kernel_ms
              << " ms\n";
