...
/*
 * 既存手法用。Green ContextやSM分割を作らず、primary context上に
 * 通常CUDA streamだけを生成する。全streamはGPU資源を共有する。
 */
inline RuntimeResources create_normal_stream_resources(
  ...
  if (scheduling_reference_sm <= 0) {
    throw std::invalid_argument(
        "scheduling_reference_sm must be positive"
    );
  }

  RA_CUDA_CHECK(cudaSetDevice(0));
  ...
  rr.sm_count_each_stream.resize(
      static_cast<std::size_t>(stream_count),
  ...
  // 全streamが通常streamであり、GC streamは存在しない。
  ...
  std::cout
      << "[normal stream setup] "
      << stream_count
      << " streams, no Green Context\n";

  return rr;
}

/*
 * stream 0は通常streamとして作成し、stream 1以降のみGCにする。
 */
inline RuntimeResources create_runtime_resources(
  ...
  /*
   * 1 streamの場合はGreen Contextを作らず、
   * primary context上の通常streamだけを作る。
   */
  if (stream_count == 1) {
    if (stream_sm_counts.at(0) != info.available_sm) {
      throw std::invalid_argument(
    ...
    std::cout
        << "[normal stream setup] no Green Context\n";
...
inline void print_runtime_resource_state(
  ...
  for (int stream_id = 0;
    ...
    if (rr.gc_stream_ids.empty()) {
      std::cout << "[normal] ";
    } else if (stream_id == rr.normal_stream_id) {
      std::cout << "[normal] ";
