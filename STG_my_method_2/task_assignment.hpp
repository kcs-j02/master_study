...
inline std::int64_t get_task_proc_time(
  ...
  /*
   * 評価用のkernel-aware時間モデル。
   * LIGHTはwork_units回、HEAVYは1反復で3段の演算を行うため、
   * 実際に投入する演算量に近い重みをbottom levelと配置へ使う。
   */
...
inline StreamScheduleResult simulate_stream_assignment(
  ...
  /*
   * bottom-levelの高いタスクを集める優先stream。
   * 通常stream固定ではなく、最大SMを持つstreamを選ぶ。
   */
  const int priority_stream_id =
      static_cast<int>(
          std::distance(
  ...
  while (!ready_task_ids.empty()) {
     ...
     * 最大SMのstreamへ配置できる場合は、そこを優先して密にする。
     */
    ...
    if (priority_stream_finish_time <=
