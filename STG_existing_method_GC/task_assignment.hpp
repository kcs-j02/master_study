...
/*
 * GH100（114 SM）向けの仮SM割り当て。
 *
 * stream 0はprimary context上の通常stream、
 * stream 1以降はGreen Contextとして扱う。
 * この配分はタスク割り当ての予測にだけ使用し、
 * 最終SM配分は後段のSM再割り当てで決定する。
 */
...
inline StreamScheduleResult simulate_stream_assignment(
  ...
  while (!ready_task_ids.empty()) {
    ...
    /*
     * STG_existing_methodから引き継いだstream 0への充填規則。
     * この比較条件ではstream 0もGreen Contextである。
     * 全streamを比較した後、現在の部分makespanを増やさずに
     * stream 0へ配置できる場合は、stream 0を優先して密にする。
     */
    double normal_stream_start_time =
        std::numeric_limits<double>::max();
    double normal_stream_finish_time =
        std::numeric_limits<double>::max();
    ...
    for (int stream_id = 0;
      ...
      /*
       * 小さいgridは全SMを利用できない。
       * そのため物理SM数ではなく、このタスクが実際に利用可能な
       * effective SM数で処理時間を見積もる。
       *
       * 例: parallel_sm_limit=64の場合
       *   114-SM stream -> effective 64 SM
       *    82-SM stream -> effective 64 SM
       *    16-SM stream -> effective 16 SM
       */
      ...
      if (stream_id == 0) {
        normal_stream_start_time = start_time;
        normal_stream_finish_time = finish_time;
      }
    ...
    /*
     * 最速streamを選んだ場合の部分makespanと比べて悪化しないなら、
     * GC stream 0へ寄せる。stream 0上では
     * find_earliest_insertion_startが最初の空き区間を選んでいるため、
     * 既存区間の隙間も優先的に埋められる。
     */
    const double best_partial_makespan =
        std::max(result.makespan, best_finish_time);

    if (normal_stream_finish_time <=
        best_partial_makespan + epsilon) {
      best_stream = 0;
      best_start_time = normal_stream_start_time;
      best_finish_time = normal_stream_finish_time;
    }
