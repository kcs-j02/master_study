...
inline RuntimeResources create_runtime_resources(
  ...
  /*
   * Green Contextを使わない通常stream 1本の構成。
   * SMを分割しないため、primary contextが全SMを利用できる。
   */
  if (stream_count == 1) {
    if (stream_sm_counts.at(0) != info.available_sm) {
      throw std::invalid_argument(
          "normal-only candidate must use all available SMs"
      );
    }

    RuntimeResources rr;

    rr.streams.resize(1, nullptr);
    rr.green_ctxs.resize(1, nullptr);
    rr.resource_descs.resize(1, nullptr);
    rr.sm_count_each_stream = stream_sm_counts;
    rr.priority_each_stream.resize(1, 0);
